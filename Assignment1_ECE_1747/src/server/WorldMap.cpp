...
Uint32 num_of_attack_requests = 0;
Uint32 num_of_use_gameobj_requests=0;
...
void WorldMap::useGameObject(Player* p)
{
	...
	if ( o )
	{
       ...
       num_of_use_gameobj_requests++;
       //printf("Use GameObj request No: %d \n", num_of_use_gameobj_requests);
...
void WorldMap::attackPlayer(Player* p, int attack_dir)
{
	...
	if ( sd->display_actions )		printf("Player %s attacks %s\n", p->name, p2->name);
       ...
       num_of_attack_requests++;
       //printf("Attack request No: %d \n", num_of_attack_requests);
...
void WorldMap::balance_lightest()
{
   ...
   list<int> overloaded_threads;
   list<int> lightly_loaded_threads;

   for (int i=0; i < sd->num_threads; i++) {
   // Go through players array for each region, i.e players[0] corresponds to players for region 0 and check if its overloaded or lightly loaded
    if (isOverloaded(players[i].size())) {
      //Overloaded, hence push this into list of overloaded threads
       overloaded_threads.push_back(i);
    } else if (isLightlyLoaded(players[i].size())) {
       // Lightly loaded, hence push this into list of lightly loaded threads
         lightly_loaded_threads.push_back(i);
    } else {
       // neither overloaded nor lightly loaded, hence do nothing
    }
  }


              printf("List of overloaded threads:\n");
	       print_list(overloaded_threads);
	       printf("List of lightly loaded threads:\n");
	       print_list(lightly_loaded_threads); 
	

    //Now find the heaviest and lightest threads from the above list

    int heaviest = (overloaded_threads.size()==0) ? -1 : find_heaviest(overloaded_threads);
    int lightest = (lightly_loaded_threads.size()==0) ? -1 : find_lightest(lightly_loaded_threads);


              printf("Heaviest thread id: %d\n", heaviest);
              printf("Lightest thread id: %d\n", lightest);
 
    if (heaviest == -1) {
         printf("[INFO]: Unable to find heaviest thread in list overloaded_threads so far ....\n");
     }

     if (lightest == -1) {
         printf("[INFO]: Unable to find lightest thread in list lightly_loaded_threads so far .... \n");
     }
   
   // Now copy all regions of heaviest thread to a bin first
       list<Region*> hRbin;
	Region* rptr;
	...
	for(int i = 0; i < n_regs.x; i++)
	{
		...
		{      
      rptr = &regions[i][j];
      fprintf(fp,"[INFO] BEFORE REASSIGNMENT: Region @ %d, %d : thread %d & %d clients \n" ,i , j , rptr->layout , rptr->n_pls);            
      if (rptr->layout == heaviest) {
      
				hRbin.push_back(rptr);
			}
		}
	}
       // Now try to adjust players in each region as much as possible as the afforded room. The room would be the safety level minus the size of the lightest thread. 
       // This is to ensure maximum utilization while avoiding going over the safety threshold
    
       //safety load is target load after load shedding
       double safety_level = (sd->light_level + sd->overloaded_level)/2;
       fprintf(fp,"SAFETY LEVEL %f\n" , safety_level);

       int reassignSize = 0;
       int room = 0;
  	   list<Region*> reassignedBin;
      
	if (lightest >= 0)
 {
     room = safety_level;
  	for (list<Region*>::iterator it=hRbin.begin(); it != hRbin.end(); ++it) {
  		if ((reassignSize + (*it)->n_pls) <= room) 
  		{
        fprintf(fp,"[INFO]: REGIONS CO-ORDS %d,%d\n" ,(*it)->pos.x , (*it)->pos.y); 
        fprintf(fp,"[INFO]: MOVING FROM HEAVIEST %d TO LIGHTEST %d\n" ,heaviest , lightest);
        printf("[INFO]: MOVING FROM HEAVIEST %d TO LIGHTEST %d\n" ,heaviest , lightest);
  			reassignSize += (*it)->n_pls;
  			reassignedBin.push_back(*it);
  		}
       ...
       // Reassignment of players within each region done. Now copy all the regions to lightest thread.
	if (lightest >-1)
 {
    for (list<Region*>::iterator it=reassignedBin.begin(); it != reassignedBin.end(); ++it) {
  		reassignRegion(*it, lightest);
  	}
	}	
       //Region + thread + player counts after the reassignment
       for(int i = 0; i < n_regs.x; i++) 
	{
		for (int j = 0; j < n_regs.y; j++)
		{
			...
			fprintf(fp,"[INFO]: AFTER REASSIGNMENT: Region @ %d, %d : thread %d & %d clients \n" , i  , j , rptr->layout , rptr->n_pls); 
		}
	}
...
int WorldMap::find_heaviest( list<int> overloaded_threads ) {
   int max = -1;
   int heaviest;

    for (list<int>::iterator p=overloaded_threads.begin(); p!=overloaded_threads.end(); ++p) {
      if (players[(*p)].size()> max) {
         max = players[(*p)].size();
         heaviest = *p;
      }
   }
...
int WorldMap::find_lightest( list<int> lightly_loaded_threads ) {

   int min = 99999;
   int lightest;

    for (list<int>::iterator p=lightly_loaded_threads.begin(); p!=lightly_loaded_threads.end(); ++p) {
      if (players[(*p)].size()< min) {
         min = players[(*p)].size();
         lightest = *p;
      }
   }
...
void WorldMap::print_list(list<int> MList) 
{
